...
cac 在端口 443 上启动一个本地 HTTPS 服务器，使用通过 `NODE_EXTRA_CA_CERTS` 信任的证书，将 `api.anthropic.com` 添加到 `/etc/hosts` 指向 `127.0.0.1`，并将其添加到 `NO_PROXY`。健康检查立即以 HTTP 200 完成。3 秒后，绕过被清理。
