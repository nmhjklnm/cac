...
Claude Code 在启动时会 ping `api.anthropic.com`。通过代理时，Cloudflare 经常返回 403（Node.js TLS 指纹被拒绝）。cac 通过临时运行一个本地 HTTPS 服务器（端口 443，使用受信任的证书）来绕过此问题，使健康检查立即完成。

这是自动的，仅在每次 `claude` 会话的前几秒内活跃。
