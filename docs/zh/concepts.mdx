...
配置了代理时，cac 始终在 `127.0.0.1` 上启动本地 TCP 中继。如果检测到 TUN 模式的代理软件（Clash、Surge、Shadowrocket、sing-box），cac 还会自动为上游代理 IP 添加直连路由，确保中继流量绕过 TUN 接口。
