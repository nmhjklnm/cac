    ...
    if [[ -n "$proxy" ]]; then
        ...
        if [[ "$has_conflict" == "true" ]]; then
            ...
            if _relay_route_ok "$proxy"; then
                ...
                # 直接修复：添加直连路由
                echo
                echo "  $(_yellow "⚠") 检测到 TUN 活跃，需要添加直连路由以确保代理连通。"
                if _relay_add_route "$proxy"; then
                    summary_parts+=("TUN 直连路由 ✓（已修复）")
                else
                    problems+=("直连路由添加失败 — 请手动执行 sudo route add -host <proxy_ip> <gateway>")
                fi
            fi
        fi
        fi
