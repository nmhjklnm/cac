...
cmd_check() {
    ...
    if [[ -n "$proxy" ]]; then
        if ! _proxy_reachable "$proxy"; then
            ...
            local _urls="https://api.ipify.org https://api.ipify.org https://api.ipify.org https://ipinfo.io/ip https://ifconfig.me"
            ...
            if [[ -n "$proxy_ip" ]]; then
                ...
                printf "\r    $(_green "✓") exit IP    $(_dim "run again to detect exit IP")           \n"
            fi
