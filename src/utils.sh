...
# Protocol-aware proxy validation: verifies the proxy is actually
# processing requests, not just listening on a port.
_proxy_check() {
    local proxy_url="$1" timeout_sec="${2:-3}"
    node -e "
(function() {
  var url = process.argv[1];
  var timeout = Number(process.argv[2]) * 1000;
  var targetHost = 'api.ipify.org';
  var targetPort = 443;
  var parsed = new URL(url);
  var proto = parsed.protocol.replace(/:$/, '').toLowerCase();
  var host = parsed.hostname;
  var port = parseInt(parsed.port || (proto === 'https' ? '443' : ((proto === 'socks5' || proto === 'socks5h') ? '1080' : '80')), 10);
  ...
  function sendSocksConnect() {
    ...
    if (hostBuf.length > 255) finish(1);
  ...
  function checkSocks5() {
    var hasAuth = user && pass;
    ...
    sock.on('data', function(chunk) {
      ...
      if (state === 'greeting') {
        ...
        var method = buf[1];
        buf = buf.slice(2);
        if (method === 0xFF) finish(1);
        if (method === 0x02) {
          if (!hasAuth || user.length > 255 || pass.length > 255) finish(1);
          var uBuf = Buffer.from(user);
          var pBuf = Buffer.from(pass);
        ...
        if (buf.length < 2) return;
        if (buf[1] !== 0x00) finish(1);
        buf = buf.slice(2);
        sendSocksConnect();
        state = 'connect';
      } else if (state === 'connect') {
        if (buf.length < 4) return;
        finish(buf[1] === 0x00 ? 0 : 1);
      }
    });
  }
  ...
  function checkHttpConnect() {
    ...
    if (user) {
      var cred = Buffer.from(user + ':' + pass).toString('base64');
  ...
  if (proto === 'https') {
    sock = tls.connect({host: host, port: port, servername: host, rejectUnauthorized: false}, onConnect);
  } else {
    sock = net.connect({host: host, port: port}, onConnect);
  }
  sock.setTimeout(timeout);
  sock.on('error', function() { finish(1); });
  sock.on('timeout', function() { finish(1); });
})(undefined);
